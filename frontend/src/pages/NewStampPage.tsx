      ...
      const result = await stampApi.upload(
        {
          ...
          reliefHeight,
          minLineWidth,
          autoProcess: mode === 'auto', // só processa auto no modo auto
        },
        setUploadProgress
      )

      // 2. Se modo SVG, enviar o SVG e pular direto para 3D
      if (mode === 'svg' && svgFile) {
        ...
        <div className="space-y-4">
                ...
                {svgFile ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-10 text-center">
                    ...
                    <p className="text-xs text-white/25 mt-1">
                      Gerado pelo ChatGPT ou Illustrator
                    </p>
                  </div>
                )}
              </div>
          ...
          {showSettings && (
            <div className="rounded-xl border border-white/8 bg-white/2 p-4 space-y-4">
              {[
                { label: 'Diâmetro (mm)', value: diameter, set: setDiameter, min: 20, max: 100, step: 5 },
          ...
          <button onClick={handleProcess} disabled={!canProcess || isUploading || isProcessing}
            className={clsx(
              'w-full py-3.5 rounded-xl font-semibold text-sm flex items-center justify-center gap-2.5 transition-all',
          ...
          {mode === 'svg' && !svgFile && (
            <div className="rounded-xl border border-violet-500/15 bg-violet-500/5 p-4">
              ...
              <ol className="space-y-1.5 text-xs text-violet-400/50 list-decimal list-inside">
                <li>Envie sua imagem para o ChatGPT</li>
                <li>Peça para gerar uma "line art SVG vetorial"</li>
                <li>Baixe o SVG gerado</li>
                <li>Faça upload aqui — vai direto para o 3D</li>
              </ol>
            </div>
          )}
        </div>
